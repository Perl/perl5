...
Before editing a file that looks generated, replace the angle-bracket placeholders and search for its generator:
...
```sh
rg -n "^(=head1|=head2) (NAME|DESCRIPTION|SYNOPSIS)\\b" pod/perl*.pod lib/*.pod Porting/*.pod
rg -n "make test|test_harness|TEST_ARGS|TEST_FILES|TEST_JOBS" README INSTALL pod/perlhack.pod Makefile.SH
rg -n "DO NOT EDIT|Generated from|regen(_[a-z]+)?|manisort|buildtoc|pod_rules" Makefile.SH regen Porting t/porting
rg -n "AUTHORS|updateAUTHORS|Maintainers|customized" AUTHORS Porting/updateAUTHORS.* Porting/Maintainers* t/porting/customized.*
```
