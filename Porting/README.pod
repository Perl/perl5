...
Check where the symbols defined in the various F<config.sh>-clones are
being used.  Useful for hunting where and wether the symbols are being
used.  VMS is probably not handled properly here, due to their own
