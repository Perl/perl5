...
Somewhat frequently, under at least some Configure options, a function
doesn't use a parameter it is passed.  This can generate compiler
warnings that should be investigated.  Perhaps the parameter isn't
actually needed and can be removed.  But often, the investigation shows
that the parameter is indeed needed; for example it is for future use,
or the function is public and its interface can't be changed, or some
Configurations do actually use it.  In these cases, we want to silence
this particular warning instance, without affecting other instances
which may crop up.

To do this, the macro L<C<PERL_UNUSED_ARG>|perlapi/PERL_UNUSED_ARG> was
invented.  It is placed in the C code in the Configurations where the
parameter doesn't get used, and silences the particular warning about a
particular argument in that particular build Configuration.

However, use of the macro can detract from the readability of the code
logic, and some compilers allow a different, less intrusive method of
accomplishing the same thing, and the C23 Standard enshrines that other
method.  So a new scheme has been invented which can replace the use of
C<PERL_UNUSED_ARG> in many cases.  There are two forms, detailed in
L<perlintern/UNUSED> and L<perlintern/DEBUG_ONLY>.

This task is to, little by little, go through the perl source code and
check occurrences of C<PERL_UNUSED_ARG> for possible changing.  As of
September 2026, there are over 400 of these.  Even determining the best
outcome, without doing any actual changes, would be helpful.

The possible outcomes are

=over 4

=item 1) If the function never uses the argument under any build
Configuration

=over 4

=item a) If function C<foo> has a C<PERL_ARGS_ASSERT_foo> macro at its
start

Convert to use L<perlintern/UNUSED>, removing the C<PERL_UNUSED_ARG>
call.

=item b) Otherwise

This generally means the function is a static function, not visible
outside the file it appears in.  It is usually best to add an entry for
the function to F<embed.fnc>, which will cause a C<PERL_ARGS_ASSERT>
macro to be generated for it, which you should add a call to, and then
follow item C<a)>.  Otherwise, leave the C<PERL_UNUSED_ARG> call as-is.

=back

=item 2) If the function always uses the argument under the DEBUGGING
build Configuration, but never otherwise

Follow the instructions for item C<1)>, but use L<perlintern/DEBUG_ONLY>
instead of C<UNUSED>.

=item 3) Otherwise

C<PERL_UNUSED_ARG> should continue to be used.  You should adjust any
calls to it to occur in just the Configurations it is appropriate for.
