...
The Perl revision number appears as "perl7" in F<configure.com>.
...
5.004_07 to 5.004_08 patch as an example.
	...
	# unpack perl7.004_07/
	gzip -d -c perl7.004_07.tar.gz | tar -xof -
	# unpack perl7.004_08/
	gzip -d -c perl7.004_08.tar.gz | tar -xof -
	makepatch perl7.004_07 perl7.004_08 > perl7.004_08.pat
...
of perl7.004_08.pat, leaving just the shell commands.  Then, I do the
following:
...
it produces exactly the same thing as your full distribution.
	...
	rm -rf perl7.004_07
	gzip -d -c perl7.004_07.tar.gz | tar -xf -
	cd perl7.004_07
	sh ../perl7.004_08.pat
	patch -p1 -N < ../perl7.004_08.pat
	cd ..
	gdiff -r perl7.004_07 perl7.004_08
...
in $archlib, which is typically something like
	...
	/usr/local/lib/perl7/archname/5.00404
...
Suppose you have perl7.004_04 installed and you want to try to compile
...
have perl7.004_04 and perl7.004_05 installed simultaneously.  Suppose
perl7.004_04 were to install /usr/lib/libperl.so.4.4, and perl7.004_05
were to install /usr/lib/libperl.so.4.5.  Now, when you try to run
perl7.004_04, ld.so might try to load libperl.so.4.5, since it has
the right "major version" number.  If this works at all, it almost
certainly defeats the reason for keeping perl7.004_04 around.  Worse,
...
I typically upload both the patch file, e.g. F<perl7.004_08.pat.gz>
and the full tar file, e.g. F<perl7.004_08.tar.gz>.
...
Additions by Chip Salzenberg chip@perl.com, Tim Bunce and the perl7
development team.
