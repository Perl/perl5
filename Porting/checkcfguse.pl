...
# (2) greps for their use in the core files,
#     if no uses (note: can have false matches),
#     outputs "-"
#
# Beware of false matches.
#
...
while (<$mani>) {
  if (/^(\S+)\s+/) {
    my $fn = $1;
    next if $fn =~ m{^(?:config_h.SH|Configure|configure\.com|Porting/(?:config|Glossary)|(?:NetWare|plan9|win32)/(?:config|(?:GNU)?[Mm]akefile)|uconfig)};
