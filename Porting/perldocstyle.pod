...
This guide will hereafter refer to user-manual section files as I, per Unix convention.
...
The author intends this early draft as a starting place from which to
...
and modules. This use of Pod has its own style guide, outlined by L<C<perlpodstyle>|perlpodstyle>.

=head2 Choice of language: American English

Perl's core documentation is written in American English. (The Chinese,
Japanese, and Korean-language README files included with Perl's source
distributions provide an exception to this--but these documents fall
outside the scope of this guide.)

Community-authored translations of Perl's documentation do exist,
covering a variety or languages. While the Perl project appreciates
...
Many of these requirements originate with
L<C<perlpodstyle>|perlpodstyle>, which derives its recommendations in
turn from long-time Unix man page conventions.
...
This section should, one way or another, also make clear the audience
that the page addresses, especially if it has expectations about the
reader's prior knowledge. For example, a man page that dives deep into
the inner workings of Perl's regular expression engine should state its
assumptions up front--and quickly redirect readers who are instead
looking for a more basic reference or tutorial.

Reference pages, when appropriate, can precede the DESCRIPTION with a
SYNOPSIS section that lists, within one more code blocks, some very
...
Further include as many subsections beneath them as needed to make the
standards of clarity, accessibility, and cross-reference affinity, L<as
suggested elsewhere in this guide|/Apply one of the four documentation modes>.

=head3 Author and copyright

Perl's core man pages inherit the copyright and license
information of Perl itself. They should not provide any
additional such information.

Man pages do have the option to credit their primary
author, or include a list of significant contributors. Note that the
presence of authors' names does not preclude a given page from L<writing
in a voice consistent with the rest of Perl's documentation|/The
documentation speaks with one voice>.
...
examples must adhere to the code-layout principles set out by Damian
Conway in chapter 2 of  I<Perl Best Practices> (O'Reilly Media, 2005).
This means four-space indentation, K&R-style block-bracketing, and so
on. (The rare code examples written expressly to demonstrate alternate
Perl syntax are exempt from this.)

L<As with CMOS|/Choice of underlying style guide: CMOS>, access to I<Perl
Best Practices> is not a prerequisite to making Perl documentation
contributions; the doc team will help contributors standardize their
code samples when needed.
...
do so by using sublists, with C<=item> elements as header text.
...
Aside from "meta" documentation such as C<perlhist> or C<perllicense>,
...
inspires comprehension by letting its readers immediately experience (and
experiment on) the concept in question.
...
Perl's own best example of a reference work is C<perlfunc>, the sprawling
...
information in the same order as every other.
...
complex function might resemble an explainer in places (c.f. [open]).
Rather, it makes sure that the authors and contributors of any given man
page agree on what sort of audience that page addresses.
...
Perl's four man pages about Unicode--comprising a short tutorial, a
...
knowledge about any other technology, or even programming in general --
especially in tutorial or introductory material.
...
=head2 Submit only complete documentation
...
=head3 Keep the past in the past
...
but should otherwise keep it in the past. A manual uncluttered with
...
should avoid explicit first-person statements, or similar self-reference
to any individual's contributor's philosophies or experiences.
...
Every line in the docs--whether English sentence or Perl statement --
should serve the purpose of bringing understanding to the reader. Should
a sentence exist mainly to make a wry joke that doesn't further the
reader's knowledge of Perl, set it aside, and consider recasting it into
a personal blog post or other article instead.
...
B<Perl 5 Porters, the; porters, the; p5p>
...
Perl's "sister language", and the current open-source language
project led by Larry Wall, Perl's original designer. Perl's
documentation should always refer to this language as "Raku" and not
"Perl 6", its former name.
