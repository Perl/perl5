...
Almost all code should use only C<utf8_to_uv>, C<perl_utf8_to_uv>,
...
by Unicode's Corregendum #9, which rejects only surrogates.  C<perl_utf8_to_uv>
accepts all syntactically valid UTF-8, as extended by Perl to allow 64-bit code
points to be encoded.

C<utf8_to_uv> is merely a synonym of C<perl_utf8_to_uv> whose name explicitly
indicates that it accepts Perl-extended UTF-8.  Perl programs
...
If you do have a plan for handling malformed input, you could instead write:
 ...
 while (s < e) {
     ...
     if (UNLIKELY(! utf8_to_uv(s, e, &cp, &advance)) {
         <bail out, discard, or convert to handleable>
     }
...
C<perl_utf8_to_uv>).  They are C<utf8_to_uvchr> and C<utf8_to_uvchr_buf>.
...
C<utf8n_to_uvchr> is more like a generalization of C<utf8_to_uvchr_buf>, with
fewer quirks, and a different method of specifying the bytes it is allowed to
examine in C<s>.  It has a C<curlen> parameter instead of a C<e> parameter,
...
But if C<flags> contains C<UTF8_DISALLOW_ILLEGAL_INTERCHANGE>, all UTF-8
sequences are excluded that evaluate to code points not recognized by Unicode
to be valid to be freely exchangeable between processes.  These are surrogate
code points, non-character code points, and all code points above U+10FFFF.
(These classes can be excluded individually by the flags
C<UTF8_DISALLOW_SURROGATE>, C<UTF8_DISALLOW_NONCHAR>, and
C<UTF8_DISALLOW_SUPER> (meaning above the legal Unicode maximum).  Use
C<UTF8_DISALLOW_ILLEGAL_C9_INTERCHANGE> to use the strictness definition given
by L<Unicode Corrigendum
#9|https://www.unicode.org/versions/corrigendum9.html>.  The difference between
traditional strictness and C9 strictness is that the latter does not forbid
non-character code points.  (They are still discouraged, however.)  For more
discussion see L<perlunicode/Noncharacter code points>.
...
Unicode (currently) disallows code points above 0x10FFFF.  But Perl predates
Unicode, and earlier standards allowed for code points up through 0x7FFF_FFFF
(2**31 - 1).  Perl, of course, would like you to be able to represent any code
point available on the platform in UTF-8.  To do so, some extension must be
used to express them.  Perl uses a natural extension to UTF-8 to represent the
ones up to 2**36-1, and invented a further extension to represent even higher
ones, so that any code point that fits in a 64-bit word can be represented.
We lump both of these extensions together and refer to them as Perl extended
UTF-8.  There exist other extensions that people have invented, incompatible
with Perl's.
...
above-Unicode code points are generally accepted but still exclude the ones
that require this extension to represent.  Use C<UTF8_WARN_PERL_EXTENDED> and
C<UTF8_DISALLOW_PERL_EXTENDED>, but not C<UTF8_WARN_PERL_SUPER> nor
C<UTF8_DISALLOW_PERL_SUPER>.
...
All such flags have C<_ALLOW_> in their names.  This stems from a time when
earlier perl versions kind-of, sort-of tried, mostly unsuccessfully, to accept
these malformations in input strings.  That ended by Perl v5.14.
...
the C<UTF8_CHECK_ONLY> flag to suppress any warnings, and then examine the
C<*errors> return.
