...
#define	EXACTF                	41	/* 0x29 Match this string using /id rules (w/len); (string not UTF-8, not guaranteed to be folded). */
#define	EXACTFL               	42	/* 0x2a Match this string using /il rules (w/len); (string not guaranteed to be folded). */
#define	EXACTFU               	43	/* 0x2b Match this string using /iu rules (w/len); (string folded iff in UTF-8; non-UTF8 folded length <= unfolded). */
#define	EXACTFAA              	44	/* 0x2c Match this string using /iaa rules (w/len) (string folded iff in UTF-8; non-UTF8 folded length <= unfolded). */
#define	EXACTFUP              	45	/* 0x2d Match this string using /iu rules (w/len); (string not UTF-8, not guaranteed to be folded; and its Problematic). */
#define	EXACTFLU8             	46	/* 0x2e Like EXACTFU, but use /il, UTF-8, folded, and everything in it is above 255. */
#define	EXACTFAA_NO_TRIE      	47	/* 0x2f Match this string using /iaa rules (w/len) (string not UTF-8, not guaranteed to be folded, not currently trie-able). */
#define	EXACT_ONLY8           	48	/* 0x30 Like EXACT, but only UTF-8 encoded targets can match */
...
#define	GROUPPN               	82	/* 0x52 Whether the group matched. */
...
#define	GROUPP                	96	/* 0x60 Whether the group matched. */
#define	OPTIMIZED             	97	/* 0x61 Placeholder for dump. */
#define	PSEUDO                	98	/* 0x62 Pseudo opcode for internal use. */
#define	LONGJMP               	99	/* 0x63 Jump far away. */
#define	BRANCHJ               	100	/* 0x64 BRANCH with long offset. */
#define	IFMATCH               	101	/* 0x65 Succeeds if the following matches; non-zero flags "f", next_off "o" means lookbehind assertion starting "f..(f-o)" characters before current */
#define	UNLESSM               	102	/* 0x66 Fails if the following matches; non-zero flags "f", next_off "o" means lookbehind assertion starting "f..(f-o)" characters before current */
#define	SUSPEND               	103	/* 0x67 "Independent" sub-RE. */
#define	IFTHEN                	104	/* 0x68 Switch, should be preceded by switcher. */
#define	RENUM                 	105	/* 0x69 Group with independently numbered parens. */
	/* ------------ States ------------- */
...
#else
EXTCONST U8 PL_varies_bitmask[] = {
    0x00, 0x00, 0x00, 0x00, 0x60, 0x00, 0xE0, 0x0F, 0xFF, 0x03, 0x00, 0x00, 0x90, 0x01
};
#endif /* DOINIT */
