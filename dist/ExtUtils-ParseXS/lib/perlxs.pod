...
=head2 More on dealing with threads (and MULTIPLICITY)

MULTIPLICITY is a way to embed a Perl interpreter as part of an
application; it is described in
L<perlguts/How multiple interpreters and concurrency are supported>.
Multiple Perl interpreters may be embedded.

Perl's implementation of threading requires MULTIPLICITY, but not the
other way around.

MULTIPLICITY without threading means that only one thing runs at a time,
so there are no concurrency issues, but each component can affect the global
state, potentially interfering with the execution of other components.
This can happen if one component:
...
nor threading.  But wait, a surprising number of libc functions do
depend on data global to the process in some way that may not be
immediately obvious.  The section 3 ones we know about are given at the
beginning of the the file F<lock_definitions.h>.  For example, perhaps
unexpectedly, calling C<L<strtok(3)>> changes the global state of a
process, and thus needs special attention.

There are two main approaches to coping with these issues, each suitable
...
C<L</Safely Storing Static Data in XS>>.

=back

Without threading, you don't have to worry about being interrupted by
the system giving control to another thread.  With threading, you can
use the locking macros C<#define>d in F<lock_definitions.h> to prevent
such interruptions, thus making the code suitable for both plain
MULTIPLICITY and threading without needing to know which is the case.

There are some exceptions.  There are a few functions that are
completely unsuitable for threaded environments.  Some of these have
alternatives that are suitable.  A few others need to be called at least
once upon process start-up to initialize, and then subsequent calls are
thread-safe.

F<lock_definitions.h> lists all such cases.  It is a generated file that
can be customized to the platform it is running on, and so the rest of
the documentation that would otherwise be here is instead included
there.
...
=head2 Finding the existing locale for a category

POSIX does not yet define a way to get the name of the current per-thread
