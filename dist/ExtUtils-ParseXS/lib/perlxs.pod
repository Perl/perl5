...
=head2 Dealing with MULTIPLICITY and threads

When you are writing code that might be used in multithreaded
applications or embedded in an application with multiple perl instances
executing (MULTIPLICITY), you need to be aware that there are certain
library calls that are unsafe to use without special handling.

Multithreading requires MULTIPLICITY, but not the other way around.  The
case of MULTIPLICITY without threading is fairly simple.  There are
functions that return their results in a global static buffer.  Those
results must be acted upon or copied to a per-instance buffer before the
function gets called again.  The functions in the list below that are
marked with C<B> have this characteristic.

It is more complicated with threading; you have to be concerned about
the possibility of a given thread's execution being interrupted by
another thread.  Mutexes can be used to avoid such interruptions, but
all possible threads must use the same mutexes, and deadlock between
threads becomes a possibility.

Perl furnishes mutexes for each function it knows to be thread-unsafe.
If the function is named I<func>, the mutex is invoked by C<FUNC_LOCK>,
and released by C<FUNC_UNLOCK>.   They are all defined in F<perl.h> and
F<reentr_locks.h>.

But it turns out that almost all libc calls are thread-safe.  Except,
the POSIX Standard says

=over

"A thread-safe function can be safely invoked concurrently with the same
function, or with calls to any other thread-safe functions"

=back

It doesn't say anything about the behavior of an allegedly thread-safe
function executing concurrently with a thread-unsafe function.  This
means there is no requirement of thread-safety if any thread is using
any thread-unsafe function.  Fortunately, in our experience, libc
implementations keep thread-safe functions safe from interference from
other threads.  And the perl executable assumes this.
...
All such macros become no-ops when compiled without threading being enabled.
...
represents a halo (which in European culture represents purity)
representing that the substituted version is fully thread-safe (on most
platforms anyway).
...
To get this automatic substitution, add this line to your code:

 #define PERL_REENTRANT

This must follow your

 #include <perl.h>

line.  If you don't include F<perl.h>, you must instead place this line

 #include <regen/reentr.pl>

before your C<PERL_REENTRANT> definition.
...
C<L</M>> flag also appears, wrap your calls to this function with the
...
destroyed at any time by another thread executing this function or a
related one.
...
needs to be locked for exclusive access.
...
This function is considered obsolete; the notes should name the
...
The sleep() function has multiple problems.  Instead use nanosleep() if
available; otherwise usleep(), but this one is now considered obsolete.
...
=back
 ...
 localeconv()       P; for the most common uses, instead use
                       ...
                       Perl 5.28, or call POSIX::localeconv()
 localtime()        A E L M T
 lrand48()          Ꞧ
...
researching this document, omissions were discovered in the source
