...
L<xsubpp>, which was bundled with Perl release 5.8.0.

In B<xsubpp> version 2.09_01, the bulk of the code for this utility was
split out into a separate module, L<ExtUtils::ParseXS>. This module
inherited the version numbering scheme of B<xsubpp>, and since then, the
...
B<ExtUtils::ParseXS> can be installed via CPAN into an older Perl
installation, allowing newer XS syntax to be used with older Perls.

(Between 1.98_01 and 2.09_01, B<ExtUtils::ParseXS> existed as a separate
fork of B<xsubpp>, with some changes ported back and forwards with the
perl distribution's B<xsubpp>, in a confusing manner.)

This document refers to changes in XS syntax by reference to B<xsubpp>
version numbers. This should be understood as usually mapping directly to
the same version number of B<ExtUtils::ParseXS>. To determine which
version of this module was bundled with which release of Perl, you can use
the L<corelist> utility which is usually a part of the Perl installation,
e.g.
...
use other systems such as L<Inline::C> or SWIG. For creating simple
...
The L<h2xs> utility bundled with Perl can in principle be used to generate
...
B<h2xs> (as well as other tools) can also be used to generate an initial
...
from B<xsubpp> 3.01 onwards you can also add typemap declarations inline
...
An XS file is parsed by L<ExtUtils::ParseXS>, or by the B<xsubpp> utility
(which is a thin wrapper over the module), and generates a C<.c> file.
B<xsubpp> is typically called at build time from the F<Makefile> of a
...
the point of accepting nonsense as input. Since around B<xsubpp> releases
...
Since B<xsubpp> 3.06, there are a set of alternative XS types which can be
...
code is typically created automatically with B<h2xs> when you first create
...
B<ExtUtils::ParseXS> XS compiler (and its B<xsubpp> wrapper) needed to
...
F<Makefile> as C<XS_VERSION>, and from there, via a C<-DXS_VERSION=...>
...
The C<VERSIONCHECK> keyword corresponds to B<xsubpp>'s C<-versioncheck>
...
The C<PROTOTYPES> keyword corresponds to B<xsubpp>'s C<-prototypes> and
...
This keyword is present since B<xsubpp> 3.04, and its value is disabled by
default.
...
using C<s/:/_/g> (unless B<xsubpp> has been invoked with C<-hiertype>), so
...
Any other bare types without a parameter name are errors since B<xsubpp>
...
The XSUB-scoped form has been available since B<xsubpp> 1.9506, but was
...
or since B<xsubpp> 3.58, only into any declarations within the same
C<CASE> branch.
...
design flaw and should probably be avoided. Since B<xsubpp> 3.01 it's been
...
Since B<xsubpp> 3.51, alias values may refer to other alias names (or to
...
C<INTERFACE> with the C<T_PTROBJ> typemap. But note that before B<xsubpp>
...
B<xsubpp> 3.01 onwards you can also include extra typemap declarations
...
Prior to B<xsubpp> 2.09_01, C<@INC> wasn't searched, and standard files
...
Note also that L<ExtUtils::MakeMaker> usually invokes B<xsubpp> with two
...
C<My::Num> from B<xsubpp> 3.60 onwards.
...
Note that the type of C<THIS> (and, since B<xsubpp> 3.55, C<CLASS>) can be
...
option of B<h2xs> to also include a skeleton set of macros, e.g.
...
an XS module. The suggested naming scheme, as used by B<h2xs>, is to use a
...
This document covers features supported by B<xsubpp> 3.62.

=head1 AUTHOR DIAGNOSTICS

As of B<xsubpp> 3.49, a few parser warnings are disabled by default. While
developing you can set C<$ENV{AUTHOR_WARNINGS}> to true in your
environment or in your F<Makefile.PL>, or set
...
Originally written by Dean Roehrich <I<roehrich@cray.com>>.
Completely rewritten in 2025.

Maintained since 1996 by The Perl Porters, <I<perl5-porters@perl.org>>.
