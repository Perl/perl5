...
parse the XS file. For example in an XSUB declaration like

    int
    foo(int a, char *b = "),")

the parser just extracts out everything between (...) and splits on
...
Functions which create a new SV, such as as C<newSViv(i)>, return an SV
...
level. Each SV above this level has its reference count decremented.
Putting an SV on the temps stack is referred to as I<mortalising> it. It
is common to create a new SV and mortalise it at the same time: here are
some examples:
...
usually has a reference count of one. It is used by many OPs to avoid
...
with the body pointer pointing back to the head with a suitable offset so
that the SV appears to have a body with an IV or whatever at a suitable
offset.
...
then to set the C<SVf_IOK> flag in addition to the C<SVf_POK>.
...
C<SvIOK()>. In general, always use macros such as C<SvIV()>, which will do
any checking and conversion for you.
...
The equivalent of C<SvIV()> for strings is

    STRLEN len;
    char *pv = SvPV(sv, len);

(and variants) which both retrieves a string pointer and sets C<len> to
its length. Note that there is no guarantee that after this call
...
used on an array reference, the SV will be converted from a reference into
a plain string SV with an C<SvPVX()> value of C<"ARRAY(0x12345678)">, and
the array's reference count decremented.
...
Instead where necessary, references to them are pushed. You will likely
first spot such an error when you start getting "Bizarre copy of ..."
error messages.
...
declarations after statements. To work round this, the C<PREINIT> keyword
allows you to inject additional variable declaration code.
...
C<PL_sv_undef>  to be stored at C<ST(0)> (this is the same value that the
Perl function C<undef> returns), and then to immediately return.
...
count count I<one too high>, and so in addition, the XS compiler emits:
...
and returning a reference to a scalar (as opposed to the C<SV*> type,
...
C<RETVAL> will be declared with type C<SVREF>, and the XSUB will return a
reference to an integer: the perl equivalent of C<my $x = 9; return \$x>.
...
typically created automatically with F<h2xs> when you first create the
skeleton of a new distribution. See L<perlxstut> for more details.
...
XXX NB: L<SCOPE|/The SCOPE: Keyword> can technically be a file-scoped
keyword too, but is dealt with elsewhere.
...
initialization from the typemap.
...
These keywords can appear anywhere within the body of an XSUB.
