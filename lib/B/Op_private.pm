...
$bits{$_}{4} = 'OPpTARGET_MY' for qw(abs add atan2 chdir chmod chomp chown chr chroot concat cos crypt divide exec exp f_add f_divide f_multiply f_subtract flock getpgrp getppid getpriority hex i_add i_divide i_modulo i_multiply i_subtract index int kill left_shift length link log mkdir modulo multiconcat multiply nbit_and nbit_or nbit_xor ncomplement oct ord pow push rand rename right_shift rindex rmdir schomp scomplement setpgrp setpriority sin sleep sqrt srand stringify subtract symlink system time unlink unshift utime wait waitpid);
...
$bits{exp}{0} = $bf[0];
@{$bits{f_add}}{1,0} = ($bf[1], $bf[1]);
@{$bits{f_divide}}{1,0} = ($bf[1], $bf[1]);
@{$bits{f_multiply}}{1,0} = ($bf[1], $bf[1]);
@{$bits{f_subtract}}{1,0} = ($bf[1], $bf[1]);
$bits{fc}{0} = $bf[0];
...
our %ops_using = (
    ...
    OPpTARGET_MY             => [qw(abs add atan2 chdir chmod chomp chown chr chroot concat cos crypt divide exec exp f_add f_divide f_multiply f_subtract flock getpgrp getppid getpriority hex i_add i_divide i_modulo i_multiply i_subtract index int kill left_shift length link log mkdir modulo multiconcat multiply nbit_and nbit_or nbit_xor ncomplement oct ord pow push rand rename right_shift rindex rmdir schomp scomplement setpgrp setpriority sin sleep sqrt srand stringify subtract symlink system time unlink unshift utime wait waitpid)],
