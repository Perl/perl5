...
#define SV_BLESSED_RETURN(s) SV_REFTYPE_RETURN(s)
	...
	case SVt_PVLV:		
            ...
            else if (isALPHA_FOLD_EQ(LvTYPE(sv), 't'))
	...
	case SVt_PVAV:		
            SV_REFTYPE_RETURN("ARRAY"); /* An array reference (C<< @$array >>) */
	case SVt_PVHV:		
            SV_REFTYPE_RETURN("HASH");  /* A hash reference (C<< %$hash >>) */
	case SVt_PVCV:		
            SV_REFTYPE_RETURN("CODE");  /* A subroutine reference (C<< $code->() >>) */
	case SVt_PVGV:		
            if(isGV_with_GP(sv))
                ...
                SV_REFTYPE_RETURN("SCALAR");
	case SVt_PVFM:		
            SV_REFTYPE_RETURN("FORMAT"); /* A format reference (C<< *IO{FORMAT} >>) */
	case SVt_PVIO:		
            SV_REFTYPE_RETURN("IO");     /* An IO reference (C<< *STDOUT{IO} >>) */
	case SVt_INVLIST:	
            SV_REFTYPE_RETURN("INVLIST"); /* A regex char-class invlist */
	case SVt_REGEXP:	
            SV_REFTYPE_RETURN("REGEXP"); /* An executable regular expression (C<< qr/../ >>) */
	default:		
            SV_REFTYPE_RETURN("UNKNOWN"); /* This should never be seen */
	}
    }
...
Perl_sv_ref(pTHX_ SV *dst, const SV *const sv, const int ob)
{
	...
	HvNAME_get(SvSTASH(sv))
        ...
        STRLEN len;
        const char * reftype = sv_reftype_len(sv, 0, &len);
        sv_setpvn(dst, reftype, len);
    }
    return dst;
}
