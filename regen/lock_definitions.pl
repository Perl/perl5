...
while (defined (my $line = <DATA>)) {
    ...
    foreach my $use (split /\s*,\s*/, $uses) {
        ...
        $entry{needs_back_compat} = 1 if $needs_back_compat;
...
#       Standard.
#
#   e)  The character 'B' means to generate backwards compatibility macros in
#       addition to the normal ones for each use in the 'uses' column.  In
#       order to not collide with any potential names that an XS writer might
#       use, the macro names generated by this are prefixed with 'PERL_'.
#       However, previous perl releases have #defined names without that
#       prefix.  This field causes this program to generate both sets of
#       macros so the previous defined symbols remain defined.
