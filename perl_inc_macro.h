/* just define a list of macros to push elements in INC so
 * we can easily use them and change order on demand
 *
...
#  endif
    ...
    /* vendorarch is always relative to vendorlib on Windows
     * for DLL-based path intuition to work correctly */
#  if !defined(WIN32)
...
/* VMS */
         ...
         * "natural" VMS idiom for a Unix path string.  We allow each element
         * to be a set of |-separated directories for compatibility.
         */
# define _INCPUSH_PERL5LIB_ADD                      \
      char buf[256];                                \
          int idx = 0;                              \
