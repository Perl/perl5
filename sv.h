...
An enum of flags for Perl types.  These are found in the file F<sv.h> in the
...
All the others are scalar types, that is, things that can be bound to a C<$>
...
typeglob has been assigned.  Assigning to it again will stop it from being a
typeglob.  C<SVt_PVLV> represents a scalar that delegates to another scalar
behind the scenes.  It is used, e.g., for the return value of C<substr> and
for tied hash and array elements.  It can hold any scalar value, including a
typeglob.  C<SVt_REGEXP> is for regular expressions.  C<SVt_INVLIST> is for
...
same way that SVt_IV does, but this is not currently supported and seems to be
a rare use case.) C<SVt_PV> can hold C<undef>, a string, or a reference.
...
typedef enum {
        ...
        SVt_PVIO,       /* 15 */
        SVt_PVOBJ,      /* 16 */
                        /* 17-31: Unused, though one should be reserved
                         * for a freed sv, if the other 3 bits below
                         * the flags ones get allocated */
        SVt_LAST        /* keep last in enum.  used to size arrays */
} svtype;

/* *** any alterations to the SV types above need to be reflected in
 * SVt_MASK and the various PL_valid_types_* tables.  As of this writing
 * those tables are in perl.h.  There are also two affected names tables
 * in dump.c, one in B.xs, and 'bodies_by_type[]' in sv_inline.h.
 *
 * The bits that match 0xe0 are CURRENTLY UNUSED The bits above that are
 * for flags, like SVf_IOK */
...
/* Anything wanting to create a reference from clean should
   ensure that it has a scalar of type SVt_IV now: */
#  define SVt_RV        SVt_IV
#endif

/* The array of arena roots for SV bodies is indexed by SvTYPE.  SVt_NULL
 * doesn't use a body, so that arena root is re-used for HEs.  SVt_IV also
 * doesn't, so that arena root is used for HVs with struct xpvhv_aux. */
...
#define _SV_HEAD_DEBUG;                                                         \
    ...
    SV *            sv_debug_parent     /* what we were cloned
                                           from (ithreads) */
#else
#define _SV_HEAD_DEBUG
#endif
...
C<void> in their names return the SV.
...
C<SvREFCNT_inc_void> can only be used if you don't need the return value.  The
...
C<SvREFCNT_inc_simple> can only be used with expressions without side effects.
...
the upgrade if necessary.  See C<L</svtype>>.
...
/* Sadly there are some parts of the core that have pointers to
   already-freed SV heads, and rely on being able to tell that they are now
   free.  So mark them all by using a consistent macro. */
...
#define SVprv_PCS_IMPORTED  SVp_SCREAM  /* RV is a proxy for a constant
                                           subroutine in another package.  Set
                                           the GvIMPORTED_CV_on() if it needs
                                           to be expanded to a real GV */

/* SVf_PROTECT is what SVf_READONLY should have been: i.e.  modifying this SV
 * is completely illegal.  However, SVf_READONLY (via Internals::SvREADONLY())
 * has come to be seen as a flag that can be temporarily set and unset by the
 * user to indicate e.g.  whether a hash is "locked".  Now, Hash::Util et al
 * only set SVf_READONLY, while core sets both (SVf_READONLY|SVf_PROTECT) to
 * indicate both to core and user code that this SV should not be messed with.
 */
...
#define SVf_FAKE        0x01000000  /* 0: glob is just a copy 1: SV head arena
                                       wasn't malloc()ed 2: For PVCV, whether
                                       CvUNIQUE(cv) refers to an eval or once
                                       only [CvEVAL(cv), CvSPECIAL(cv)] 3: HV:
                                       informally reserved by DAPM for vtables
                                       4: Together with other flags (or lack
                                       thereof) indicates a regex, including
                                       PVLV-as-regex.  See isREGEXP(). */
...
#define SVf_BREAK       0x04000000  /* refcnt is artificially low - used by
                                       SVs in final arena cleanup.  Set in
                                       S_regtry on PL_reg_curpm, so that
                                       perl_destruct will skip it.  Used
                                       for mark and sweep by OP_AASSIGN */
...
/* SVf_AMAGIC means that the stash *may* have overload methods.
 * It's set each time a function is compiled into a stash, and is
 * reset by the overload code when called for the first time and
 * finds that there are no overload methods.  Note that this used
 * to be set on the object; but is now only set on stashes.
 */
#define SVf_AMAGIC      0x10000000  /* has magical overloaded methods */
#define SVf_IsCOW       0x10000000  /* copy on write (shared hash
                                       key if SvLEN == 0) */

/* Ensure this value does not clash with the GV_ADD* flags in gv.h, or
   the CV_CKPROTO_* flags in op.c, or the padadd_* flags in pad.h: */
#define SVf_UTF8        0x20000000  /* SvPV is UTF-8 encoded This is also set
                                       on RVs whose overloaded stringification
                                       is UTF-8.  This might only happen as a
                                       side effect of SvPV() */
/* PVHV */
#define SVphv_SHAREKEYS 0x20000000  /* PVHV keys live on shared string table */

/* PVAV could probably use 0x2000000 without conflict.  I assume that PVFM can
   be UTF-8 encoded, and PVCVs could well have UTF-8 prototypes.  PVIOs
   haven't been restructured, so sometimes get used as string buffers. */
...
#define _XPVCV_COMMON                                               \
    ...
    CV *        xcv_outside;                                        \
    U32         xcv_outside_seq; /* the COP sequence (at the point  \
                                  * of our compilation) in the      \
                                  * lexically enclosing sub */      \
...
struct xpvio {
     ...
     * suggested by Gurusamy Sarathy).  For further information see Geir
     * Johansen's problem report titled [ID 20000612.002 (#3366)] Perl
     * problem on Cray system The any pointer (known as IoANY()) will also be
...
/* The following macros define implementation-independent
   predicates on SVs. */
...
Returns a U32 value indicating whether the SV contains an integer.  Checks the
B<private> setting.  Use C<SvIOK> instead.
...
Tells an SV that it is a string and disables all other C<OK> bits.  Will also
turn off the UTF-8 status.

=for apidoc Am|U32|SvBoolFlagsOK|SV* sv
Returns a bool indicating whether the SV has the right flags set such that it
is safe to call C<BOOL_INTERNALS_sv_isbool()> or
C<BOOL_INTERNALS_sv_isbool_true()> or C<BOOL_INTERNALS_sv_isbool_false()>.
Currently equivalent to C<SvIandPOK(sv)> or C<SvIOK(sv) && SvPOK(sv)>.
Serialization may want to unroll this check.  If so you are strongly
recommended to add code like C<assert(SvBoolFlagsOK(sv));> B<before> calling
using any of the BOOL_INTERNALS macros.

=for apidoc Am|U32|SvIandPOK|SV* sv
Returns a bool indicating whether the SV is both C<SvPOK()> and C<SvIOK()> at
the same time.  Equivalent to C<SvIOK(sv) && SvPOK(sv)> but more efficient.

=for apidoc Am|void|SvIandPOK_on|SV* sv
Tells an SV that is a string and a number in one operation.  Equivalent to
C<SvIOK_on(sv); SvPOK_on(sv);> but more efficient.
...
Checks if a C<SvBoolFlagsOK()> sv is a bool.  B<Note> that it is the caller's
responsibility to ensure that the sv is C<SvBoolFlagsOK()> before calling
this.  This is only useful in specialized logic like serialization code where
performance is critical and the flags have already been checked to be
correct.  Almost always you should be using C<sv_isbool(sv)> instead.

=for apidoc Am|bool|BOOL_INTERNALS_sv_isbool_true|SV* sv
Checks if a C<SvBoolFlagsOK()> sv is a true bool.  B<Note> that it is the
caller's responsibility to ensure that the sv is C<SvBoolFlagsOK()> before
calling this.  This is only useful in specialized logic like serialization
code where performance is critical and the flags have already been checked to
be correct.  This is B<NOT> what you should use to check if an SV is "true",
for that you should be using C<SvTRUE(sv)> instead.

=for apidoc Am|bool|BOOL_INTERNALS_sv_isbool_false|SV* sv
Checks if a C<SvBoolFlagsOK()> sv is a false bool.  B<Note> that it is the
caller's responsibility to ensure that the sv is C<SvBoolFlagsOK()> before
calling this.  This is only useful in specialized logic like serialization
code where performance is critical and the flags have already been checked to
be correct.  This is B<NOT> what you should use to check if an SV is "false",
for that you should be using C<!SvTRUE(sv)> instead.
...
These return a pointer to the physical string in the SV.  The SV must contain
a string.  Prior to 5.9.3 it is not safe to execute these unless the SV's
type >= C<SVt_PV>.
...
C<SvPVX_mutable> is merely a synonym for C<SvPVX>, but its name emphasizes
that the string is modifiable by the caller.

C<SvPVX_const> differs in that the return value has been cast so that the
compiler will complain if you were to try to modify the contents of the
string, (unless you cast away const yourself).

=for apidoc Am|STRLEN|SvCUR|SV* sv
Returns the length, in bytes, of the PV inside the SV.  Note that this may not
match Perl's C<length>; for that, use C<sv_len_utf8(sv)>.  See C<L</SvLEN>>
also.
...
which is in the SV, where there is usually a trailing C<NUL> character (even
though Perl scalars do not strictly require it).  See C<L</SvCUR>>.  Access
the character as C<*(SvEND(sv))>.

Warning: If C<SvCUR> is equal to C<SvLEN>, then C<SvEND> points to unallocated
memory.
...
Set the value of the IV pointer in sv to val.  It is possible to perform the
same function of this macro with an lvalue assignment to C<SvIVX>.  With
future Perls, however, it will be more efficient to use C<SvIV_set> instead of
the lvalue assignment to C<SvIVX>.
...
mischief, so do C<SvOOK_off(sv)> and use C<sv_force_normal> or C<SvPV_force>
(or check the C<SvIsCOW> flag) first to make sure this modification is safe.
Then finally, if it is not a COW, call C<L</SvPV_free>> to free the previous
PV buffer.
...
#define SvOK_off(sv)                        \
    (assert_not_ROK(sv) assert_not_glob(sv) \
     SvFLAGS(sv) &= ~(SVf_OK|               \
                      SVf_IVisUV|SVf_UTF8), \
                            SvOOK_off(sv))
#define SvOK_off_exc_UV(sv)         \
    (assert_not_ROK(sv)             \
     SvFLAGS(sv) &= ~(SVf_OK|       \
                      SVf_UTF8),    \
                            SvOOK_off(sv))
...
#define BOOL_INTERNALS_sv_isbool(sv)    \
    (SvIsCOW_static(sv) &&              \
...
Unsets the UTF-8 status of an SV (the data is not changed, just the flag).  Do
not use frivolously.

=for apidoc Am|void|SvPOK_only_UTF8|SV* sv
Tells an SV that it is a string and disables all other C<OK> bits, and leaves
the UTF-8 status as it was.

=cut
*/

/* Ensure the return value of this macro does not
   clash with the GV_ADD* flags in gv.h: */
...
Returns a boolean as to whether C<sv> has overloading
(active magic) enabled or not.
...
then the scalar is active data, and has the potential to return a new value
every time it is accessed.  Hence you must be careful to only read it once
per user logical operation and work with that returned value.  If neither
is true then the scalar's value cannot change unless written to.
...
#define SvPCS_IMPORTED(sv)                          \
    ((SvFLAGS(sv) & (SVf_ROK|SVprv_PCS_IMPORTED))   \
...
A quick flag check to see whether an C<sv> should be passed to
C<sv_force_normal> to be "downgraded" before C<SvIVX> or C<SvPVX> can be
modified directly.
...
#  define SvTAIL(sv)                                        \
       ({ const SV *const _svtail = (const SV *)(sv);       \
...
/* Does the SV have a Boyer-Moore table attached as magic? 'VALID'
 * is a poor name, but is kept for historical reasons. */
...
/* Need -0.0 for SvNVX to preserve IEEE FP "negative zero"
   because +0.0 + -0.0 => +0.0 but -0.0 + -0.0 => -0.0 */
...
#ifndef PERL_POISON
/* Given that these two are new, there can't be any existing code
 * using them as LVALUEs, so prevent that from happening */
#  define SvPVX_mutable(sv)     ((char *)((sv)->sv_u.svu_pv))
#  define SvPVX_const(sv)       ((const char*)((sv)->sv_u.svu_pv))
#else
/* Except for the poison code, which uses & to scribble
   over the pointer after free() is called. */
...
Low level micro optimization of C<L</SvGROW>>.  It is generally better to
use C<SvGROW> instead.  This is because C<SvPV_renew> ignores potential
issues that C<SvGROW> handles.  C<sv> needs to have a real C<PV> that is
unencumbered by things like COW.  Using C<SV_CHECK_THINKFIRST> or
C<SV_CHECK_THINKFIRST_COW_DROP> before calling this should clean it up,
but why not just use C<SvGROW> if you're not sure about the provenance?
...
Trim any trailing unused memory in the PV of C<sv>, which needs to
have a real C<PV> that is unencumbered by things like COW.  Think
first before using this functionality.  Is the space saving really
worth giving up COW? Will the needed size of C<sv> stay the same?

If the answers are both yes, then use L</C<SV_CHECK_THINKFIRST>>
or L</C<SV_CHECK_THINKFIRST_COW_DROP>> before calling this.
...
Frees the PV buffer in C<sv>, leaving things in a precarious
state, so should only be used as part of a larger operation
...
#ifdef PERL_CORE
/* Code that crops up in three places to take a
   scalar and ready it to hold a reference */
...
expression is tainted--usually a variable, but possibly also implicit inputs
such as locale settings.  C<SvTAINT> propagates that taintedness to the
outputs of an expression in a pessimistic fashion; i.e., without paying
...
useful data from it.  For example if the SV was C<SvROK>, then the referent
will have its reference count decremented, and the SV itself may be converted
to an C<SvPOK> scalar with a string buffer containing a value such as
C<"ARRAY(0x1234)">.
...
The forms with C<mutable> in their names are effectively the same as those
without, but the name emphasizes that the string is modifiable by the caller,
which it is in all the forms.
...
C<SvPV_nolen>) can expose the SV's internal string buffer.  If that buffer
consists entirely of bytes 0-255 and includes any bytes above 127, then you
B<MUST> consult C<SvUTF8> to determine the actual code points the string is
meant to contain.  Generally speaking, it is probably safer to prefer
...
As of Perl 5.32, all are guaranteed to evaluate C<sv> only once.  Prior to
that release, only C<SvTRUEx> guaranteed single evaluation; now C<SvTRUEx> is
...
C<SvTRUE_NN> is like C<L</SvTRUE>>, but C<sv> is assumed to be non-null (NN).
If there is a possibility that it is NULL, use plain C<SvTRUE>.
...
/* When this code was written, embed.fnc could not handle function
 * pointer parameters; perhaps it still can't */
...
/* This test is "is there a cached PV that we can use directly?" We can if
 * a) SVf_POK is true and there's definitely no get magic on the scalar b)
 * SVp_POK is true, there's no get magic, and we know that the cached PV
 * came from an IV conversion.  For the latter case, we don't set SVf_POK
 * so that we can distinguish whether the value originated as a string or
 * as an integer, before we cached the second representation. */
...
/* define FOOx(): Before FOO(x) was inlined, these
 * were idempotent versions of FOO(). */
...
/* XXX This flag actually enabled for any COW.  But it appears not to do
   anything.  Can we just remove it? Or will it serve some future purpose. */
...
/* Tell sv_utf8_upgrade() to not check to see if an upgrade
 * is really needed.  This is used when the caller has
 * already determined it is, and avoids redundant work */
...
/* The core is safe for this COW optimisation.  XS code on CPAN may not
   be.  So only default to doing the COW setup if we're in the core.
 */
#ifdef PERL_CORE
...
Copy on Write (COW) actually happens.  This COW would be useless, as it
would immediately get changed to something else.  This function also removes
any other encumbrances that would be problematic when changing C<sv>.
...
#   define CAN_COW_MASK                     \
        (SVf_POK|SVf_ROK|SVp_POK|SVf_FAKE|  \
...
Remove any encumbrances from C<sv>, that need to be taken care
of before it is modifiable.  For example if it is Copy on Write
(COW), now is the time to make that copy.

If you know that you are about to change the PV value of C<sv>,
instead use L</C<SV_CHECK_THINKFIRST_COW_DROP>> to avoid the
write that would be immediately written again.

=cut
*/
#define SV_CHECK_THINKFIRST(sv) \
    if (SvTHINKFIRST(sv))       \
...
These are identical.  They create an RV wrapper for an SV.
The reference count for the original SV is incremented.
...
/* the following macros update any magic values
   this C<sv> is associated with */
...
after modifying a scalar, in case it is a magical variable like C<$|> or a
tied variable (it calls C<STORE>).  This macro evaluates its argument more
than once.
...
Arranges for C<sv> to be shared between threads if a suitable module has been
loaded.

=for apidoc Am|void|SvLOCK|SV* sv
Arranges for a mutual exclusion lock to be obtained on C<sv> if a suitable
module has been loaded.

=for apidoc Am|void|SvUNLOCK|SV* sv
Releases a mutual exclusion lock on C<sv> if a suitable module has been
loaded.

=for apidoc_section $SV

=for apidoc Am|char *|SvGROW|SV* sv|STRLEN len
Expands the character buffer in the SV so that it has room for the indicated
number of bytes (remember to reserve space for an extra trailing C<NUL>
character).  Calls C<sv_grow> to perform the expansion if necessary.  Returns
a pointer to the character buffer.  SV must be of type >= C<SVt_PV>.  One
...
Ensures that sv is a SVt_PV and that its SvCUR is 0, and that it is properly
null terminated.  Equivalent to sv_setpvs(""), but more efficient.

=for apidoc Am|char *|SvPVCLEAR_FRESH|SV* sv

Like SvPVCLEAR, but optimized for newly-minted SVt_PV/PVIV/PVNV/PVMG that
already have a PV buffer allocated, but no SvTHINKFIRST.
...
/* Is this a per-interpreter immortal SV (rather than global)? These should
 * either occupy adjacent entries in the interpreter struct (MULTIPLICITY)
 * or adjacent elements of PL_sv_immortals[] otherwise.  The unsigned
 * (Size_t) cast avoids the need for a second < 0 condition.
 */
...
These set an SV to a true or false boolean value,
upgrading first if necessary.

They differ only in that C<sv_setbool_mg> handles
'set' magic; C<sv_setbool> does not.
...
/* If I give every macro argument a different name, then there won't be
   bugs where nested macros get confused.  Been there, done that. */
/*
=for apidoc Am|bool|isGV_with_GP|SV * sv
Returns a boolean as to whether or not C<sv> is
a GV with a pointer to a GP (glob pointer).
...
characters) into it.  If C<utf8> is true, calls C<SvUTF8_on> on the
new SV.  Implemented as a wrapper around C<newSVpvn_flags>.
...
#ifdef DEBUGGING
/* Does the bot know something I don't? 10:28 <@Nicholas>
   metabatman 10:28 <+meta> Nicholas: crash
 */
#  define SvOOK_offset(sv, offset)                                          \
       STMT_START {                                                         \
           ...
           if (SvOOK(sv)) {                                                 \
               ...
               {                                                            \
                   /* Validate the preceding buffer's sentinels             \
                      to verify that no-one is using it. */                 \
 ...
 * Since the larger IV-holding variants of SVs store their integer values
 * in their respective bodies, the family of SvIV() accessor macros would
 * naively have to branch on the SV type to find the integer value either
 * in the HEAD or BODY.  In order to avoid this expensive branch, a clever
 * soul has deployed a great hack: We set up the SvANY pointer such that
 * instead of pointing to a real body, it points into the memory before
 * the location of the head.  We compute this pointer such that the
 * location of the integer member of the hypothetical body struct happens
 * to be the same as the location of the integer member of the bodyless SV
 * head.  This now means that the SvIV() family of accessors can always
 * read from the (hypothetical or real) body via SvANY.
 *
 * Since the 5.21 dev series, we employ the same trick for NVs if the
 * architecture can support it (NVSIZE <= IVSIZE).
*/
...
/* Certain cases in Perl_ss_dup have been merged, by relying on
   the fact that currently av_dup, gv_dup and hv_dup are the
   same as sv_dup.  If this changes, please unmerge ss_dup.
