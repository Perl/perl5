...
for ('kapow') {
    is( ${^ARG}, 'kapow', '${^ARG} in a foreach' );
}
is( ${^ARG}, 'zlonk', '${^ARG} in a postfix for' ) for 'zlonk';
...
# XXX these will need to change when /p is a no-op and
# the caret variables are actual aliases to the match variables
...
# these need to be magicalized if we see them before the short one
...
is( ${^OS_ERROR}, $!, '${^OS_ERROR} == $!' );
is( ${^OS_ERROR}, ${^ERRNO}, '${^OS_ERROR} == ${^ERRNO}' );
