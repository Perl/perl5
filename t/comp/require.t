...
eval {require 5.005};
print "# $@\nnot " if $@;
print "ok ",$main::i++," - require 5.005 try 1\n";

eval { require 5.005 };
print "# $@\nnot " if $@;
print "ok ",$main::i++," - require 5.005 try 2\n";

eval { require 5.005; };
print "# $@\nnot " if $@;
print "ok ",$main::i++," - require 5.005 try 3\n";

eval {
    require 5.005
};
print "# $@\nnot " if $@;
print "ok ",$main::i++," - require 5.005 try 4\n";

# new style version numbers

eval { require v5.5.630; };
print "# $@\nnot " if $@;
print "ok ",$main::i++," - require 5.5.630\n";

eval { require(v5.5.630); };
print "# $@\nnot " if $@;
print "ok ",$main::i++," - require(v5.5.630) with parens [perl #124153]\n";

sub v5 { die }
eval { require v5; };
print "# $@\nnot " if $@;
print "ok ",$main::i++," - require v5 ignores sub named v5\n";

eval { require 10.0.2; };
print "# $@\nnot " unless $@ =~ qr{\QPerl v10.0.2 required\E};
print "ok ",$main::i++," - require 10.0.2\n";

my $ver = 5.005_63;
eval { require $ver; };
print "# $@\nnot " if $@;
print "ok ",$main::i++," - require 5.005_63\n";

# check inaccurate fp
$ver = 10.2;
eval { require $ver; };
print "# $@\nnot " unless $@ =~ qr{\QPerl v10.200.0 require\E};
print "ok ",$main::i++," - require 10.2 $@\n";

$ver = 10.000_02;
eval { require $ver; };
print "# $@\nnot " unless $@ =~ qr{\QPerl v10.0.20 required\E};
print "ok ",$main::i++," - require 10.000_02\n";

print "not " unless 5.5.1 gt v5.5;
print "ok ",$main::i++," - 5.5.1 gt v5.5\n";

{
    print "not " unless v5.5.640 eq "\x{5}\x{5}\x{280}";
    print "ok ",$main::i++," - v5.5.640 eq \\x{5}\\x{5}\\x{280}\n";

    print "not " unless v7.15 eq "\x{7}\x{f}";
    print "ok ",$main::i++," - v7.15 eq \\x{7}\\x{f}\n";

    print "not "
      unless v1.20.300.4000.50000.600000 eq "\x{1}\x{14}\x{12c}\x{fa0}\x{c350}\x{927c0}";
    print "ok ",$main::i++," - v1.20.300.4000.50000.600000 eq ...\n";
}

{
    ...
    print "# $@\nnot " if $@;
    print "ok ",$main::i++," - require 5.11.0\n";
    eval 'BEGIN {require 5.11.0} ${"foo"} = "bar";';
    print "# $@\nnot " if $@;
    print "ok ",$main::i++,"\ - BEGIN { require 5.11.0}\n";
}

# interaction with pod (see the eof)
write_file('bleah.pm', "print 'ok $main::i - require bleah.pm\n'; 1;\n");
require "bleah.pm";
$main::i++;
...
print "ok ",$main::i++," - require returning 0\n";

print "not " if exists $INC{'bleah.pm'};
print "ok ",$main::i++," - %INC not updated\n";
...
for my $expected_compile (1,0) {
    ...
    print "ok ",$main::i++," - exp $expected_compile; bleah.flg\n";
    write_file('bleah.pm', "unlink '$flag_file' or die; \$a=0; \$b=1/\$a; 1;\n");
    print "# $@\nnot " if eval { require 'bleah.pm' };
    print "ok ",$main::i++," - exp $expected_compile; require bleah.pm with flag file\n";
    print "not " unless -e $flag_file xor $expected_compile;
    print "ok ",$main::i++," - exp $expected_compile; -e flag_file\n";
    print "not " unless exists $INC{'bleah.pm'};
    print "ok ",$main::i++," - exp $expected_compile; exists \$INC{'bleah.pm}\n";
}
...
print "ok ",$main::i++," - syntax error\n";

# previous failure cached in %INC
print "not " unless exists $INC{'bleah.pm'};
print "ok ",$main::i++," - cached %INC\n";
write_file($flag_file, 1);
write_file('bleah.pm', "unlink '$flag_file'; 1");
print "# $@\nnot " if eval { require 'bleah.pm' };
print "ok ",$main::i++," - eval { require 'bleah.pm' }\n";
print "# $@\nnot " unless $@ =~ /Compilation failed/i;
print "ok ",$main::i++," - Compilation failed\n";
print "not " unless -e $flag_file;
print "ok ",$main::i++," - -e flag_file\n";
print "not " unless exists $INC{'bleah.pm'};
print "ok ",$main::i++," - \$INC{'bleah.pm'}\n";

# successful require
do_require "1";
print "# $@\nnot " if $@;
print "ok ",$main::i++," - do_require '1';\n";

# do FILE shouldn't see any outside lexicals
my $x = "ok $main::i - bleah.do\n";
write_file("bleah.do", <<EOT);
\$main::x = "not ok $main::i - bleah.do\\n";
EOT
do "bleah.do" or die $@;
dofile();
...
print "ok $main::i - require() context\n";
1;
**BLEAH**
);
...
print "ok ", $main::i," - require ::bleah is banned\n";
...
if($@ =~ /Can't locate threads in \@INC/) {
    print "ok $main::i - RT #24404\n";
} else {
    print "not ok - RT #24404$main::i\n";
}
...
$main::i++;
if($@ =~ /Can't locate threads in \@INC/) {
    print "ok $main::i - [perl #24482] require CORE::foo\n";
} else {
...
if ($@ =~ /^This is an expected error/) {
    print "ok $main::i - expected error\n";
} else {
    print "not ok $main::i - expected error\n";
}
...
if (defined &DynaLoader::boot_DynaLoader) {
    ...
    if ($@ =~ /^This is an expected error/) {
	print "ok $main::i - require(func())\n";
    } else {
	print "not ok $main::i - require(func())\n";
    }
} else {
    print "ok $main::i # SKIP Cwd may not be available in miniperl\n";
}

{
    BEGIN { ${^OPEN} = ":utf8\0"; }
    %INC = ();
    write_file('bleah.pm',"package F; our \$x = '\xD1\x9E';\n");
    eval { require "bleah.pm" };
    $main::i++;
    my $not = $F::x eq "\xD1\x9E" ? "" : "not ";
    print "${not}ok $main::i - require ignores I/O layers\n";
}

{
    ...
    $main::i++;
    print "${not}ok $main::i - require does not localise %^H at run time\n";
}
...
foreach (sort keys %templates) {
    $main::i++; do_require(bytes_to_utf($_, qq(print "ok $main::i # $_\\n"; 1;\n), 1));
    if ($@ =~ /^(Unsupported script encoding \Q$_\E)/) {
	print "ok $main::i # skip $1\n";
    }
}
