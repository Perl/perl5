/*    regexp.h
 *
 *    Copyright (C) 1993, 1994, 1996, 1997, 1999, 2000, 2001, 2003, 2005,
 ...
 *
 * Caveat: this is V8 regexp(3) [actually, a reimplementation
 * thereof], not the System V one.
*/
#ifndef PLUGGABLE_RE_EXTENSION
/* we don't want to include this stuff if we are inside of an external
   regex engine based on the core one - like re 'debug' */
...
typedef struct regexp_paren_pair {
    SSize_t start;
    SSize_t end;
    /* 'start_tmp' records a new opening position before the matching
     * end has been found, so that the old start and end values are
     * still valid, e.g.  "abc" =~ /(.(?{print "[$1]"}))+/ outputs
     * [][a][b] This field is not part of the API. */
    SSize_t start_tmp;
} regexp_paren_pair;
...
/* Subtracting b from a leaves in a everything that was there that isn't
 * in b, that is the intersection of a with b's complement */
...
typedef struct regexp {
    ...
    const struct regexp_engine  *engine;                /* what engine created
                                                           this regexp? */
    REGEXP                      *mother_re;             /* what re is this
                                                           a lightweight
                                                           copy of? */
    ...
    U32                         nparens;                /* physical number of
                                                           capture buffers */
    U32                         logical_nparens;        /* logical_number of
                                                           capture buffers */
    I32                         *logical_to_parno;      /* map logical parno to
                                                           first physcial */
    I32                         *parno_to_logical;      /* map every physical
                                                           parno to logical */
    I32                         *parno_to_logical_next; /* map every physical
                                                           parno to the next
                                                           physical with the
                                                           same logical id */
    ...
    U32                         extflags;               /* Flags used both
                                                           externally and
                                                           internally */
    SSize_t                     minlen;                 /* minimum possible
                                                           number of chars in
                                                           string to match */
    SSize_t                     minlenret;              /* minimum possible
                                                           number of chars
                                                           in $& */
    STRLEN                      gofs;                   /* chars left of pos
                                                           that we search
                                                           from */
    ...
    void                        *pprivate;              /* Data private to
                                                         * the regex engine
                                                         * which created this
                                                         * object. */
    U32                         intflags;               /* Engine Specific
                                                           Internal flags */
    ...
    regexp_paren_pair           *offs;                  /* Array of offsets for
                                                           (@-) and (@+) */
    char                        **recurse_locinput;     /* used to detect
                                                           infinite recursion,
                                                           XXX: move to
                                                           internal */
    U32                         lastcloseparen;         /* last close paren
                                                           matched ($^N) */
    ...
    /* original flags used to compile the pattern, may
     * differ from extflags in various ways */
    PERL_BITFIELD32             compflags:9;

    /*---------------------------------------------------------------------- */

    char                        *subbeg;                /* saved or original
                                                           string so \digit
                                                           works forever. */
    SV_SAVED_COPY                                       /* If non-NULL, SV
                                                           which is COW from
                                                           original */
    SSize_t                     sublen;                 /* Length of string
                                                           pointed by subbeg */
    SSize_t                     suboffset;              /* byte offset of
                                                           subbeg from logical
                                                           start of str */
    SSize_t                     subcoffset;             /* suboffset equiv,
                                                           but in chars
                                                           (for @-/@+) */
    SSize_t                     maxlen;                 /* minimum possible
                                                           number of chars in
                                                           string to match */
...
/* regexp_engine structure.  This is the dispatch table for regexes.  Any
 * regex engine implementation must be able to build one of these.
 */
...
Returns a boolean indicating whether the SV (or the one
it references) is a REGEXP.

If you want to do something with the REGEXP* later use
SvRX instead and check for NULL.
...
/* This string is expected by XS_re_regexp_pattern() in
 ...
 * immediately follow; finally the used RXf-only (unshared) bits, so that
 ...
 * RXf_BASE_SHIFT and Pmf_BASE_SHIFT down by the number of shared bits you
 * added.  That's it.  Things should be binary compatible.  But if either of
 * these gets to having to subtract rather than add, leave at 0 and instead
 * adjust all the entries that are in terms of it.  But if the first one of
 * those is already RXf_BASE_SHIFT+0, there are no bits left, and a redesign
 * is in order.
 *
 ...
 * allocate off the low end until you get to RXf_BASE_SHIFT+0.  If that
 * isn't enough, move RXf_BASE_SHIFT down (if possible) and add the new bit
...
/* Stuff that needs to be included in the pluggable
   extension goes below here */
...
#define REXEC_FAIL_ON_UNDERFLOW 0x80    /* fail the match if $& would start
                                           before the start pos (so s/.\G//
                                           would fail on second iteration */
...
/* like regmatch_info_aux, but contains extra fields only needed if
 * the pattern contains (?{}).  If used, is snuck into the second
 * slot in the regmatch_state stack at the start of execution */
...
Perl_regexec_flags and then passed to regtry(), regmatch() etc.  It is
allocated as a local var on the stack, so nothing should be stored in
it that needs preserving or clearing up on croak().  For that, see the
aux_info and aux_info_eval members of the regmatch_state union.
...
typedef struct {
    ...
    I32  poscache_maxiter; /* how many whilems todo before
                              S-L cache kicks in */
...
struct next_matchable_info {
    ...
    /* The size is from trial and error, and could change with new
     * Unicode standards, in which case there is an assertion that should
     * start failing.  This size could be calculated in one of the regen
     * scripts dealing with Unicode, but khw thinks the likelihood of it
     * changing is low enough that it isn't worth the effort. */
    U8                  matches[18];
};
...
typedef struct regmatch_state {
    ...
    union {
         ...
         * represent one or two extra chunks of data that need allocating at
         * the start of a match.  These fields would logically live in the
         * regmatch_info struct, except that is allocated on the C stack,
         * and these fields are all things that require cleanup after a
         * croak(), when the stack is lost.  As a convenience, we just use
         * the first 1 or 2 regmatch_state slots to store this info, as we
         * will be allocating a slab of these anyway.  Otherwise we'd have
         * to malloc and then free them, or allocate them on the save stack
         * (where they will get realloced if the save stack grows).
         * info_aux contains the extra fields that are always needed;
         * info_aux_eval contains extra fields that only needed if the
         * pattern contains code blocks We split them into two separate
         * structs to avoid increasing the size of the union.
         */
        ...
        /* this is a fake union member that matches the first element of each
         * member that needs to store positive backtrack information */
        struct {
        ...
        struct { /* this first element must match u.yes */
            struct regmatch_state   *prev_yes_state;
            U32                     lastparen;
            U32                     lastcloseparen;
            CHECKPOINT              cp;
        ...
        struct { /* the first elements must match u.branchlike */
            struct regmatch_state   *prev_yes_state;
            U32                     lastparen;
            U32                     lastcloseparen;
            CHECKPOINT              cp;
        ...
        /* quantifiers - these members are used for storing state
        ...
        struct { /* this first element must match u.yes */
            ...
            CHECKPOINT              cp;                 /* remember current
                                                           savestack index */
            bool                    minmod;
            int                     parenfloor;         /* how far back to
                                                           strip paren data */
...
/* how many regmatch_state structs to allocate as a single slab.  We
 * do it in 4K blocks for efficiency.  The "3" is 2 for the
 * next/prev pointers, plus 1 for any mythical malloc overhead. */
