...
When present enables a special mode which stops Perl from throwing
away code in BEGIN blocks so that tools like C<Devel::Cover> can
see them. This switch may cause a significant increase in memory
usage, and should only be used in conjunction with a module like
C<Devel::Cover>. Mnemonic: "Kover mode".
