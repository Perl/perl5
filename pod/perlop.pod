...
First available in Perl 5.24.0 (the 5.10.1 and 5.10.0 versions behaved differently),
...
not all C<when> clauses call the smartmatch operator.  The smartmatch
...
the table is sorted on the right operand instead of on the left.
 ...
 ===============================================================
 Scalar    Object     invoke ~~ overloading on Object

 Scalar    undef      check whether Any is undefined                    
                like: !defined Any

 Scalar    CODE       sub passed Any returns true              
                like: CODE->(Any)

 Scalar    Regexp     pattern match
                like: Any =~ /Regexp/

 Object    Scalar     invole ~~ overloading on Object

Otherwise die.

=head3 Smartmatching of Objects

If the object, either to the right or left of the operator, doesn't
overload C<~~> then that row of the smart match table is skipped.
That's because one has no business digging around to see whether
something is "in" an object.  These are all illegal on objects without
a C<~~> overload:
