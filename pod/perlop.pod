...
As of Perl 5.32, relational and equality operators can be chained in
the way that mathematicians would expect. Thus

    if ( 1 < $x < $y == $z < 10 ) { ... }

is legal code that is mathematically equivalent to

    if ( 1 < $x && $x < $y && $y == $z && $z < 10) { ... }

with the only functional difference being that each compared expression
in the chain is evaluated once, even if it is involved in two comparisons.
...
with the only difference being that if C<expensive_function()> returned
a tied scalar its FETCH method would be called exactly once.

The implied AND operators in the chained expression behave similarly
to the unchained form in terms of short-circuiting, for instance in the
following example the function C<expensive_function()> will not be called
as the expression will short circuit immediately after the C<< 1 < $x >>
has been evaluated to be false.

    $x = 0;
    if ( 1 < $x < expensive_function() < 10 ) { ... }
