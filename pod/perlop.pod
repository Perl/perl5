...
Be aware that the following may not behave in the fashion that you
expect-

    $a = 'echo test';
    $a =~ m"$a";      # interpolates $a
    $a =~ m'$a';      # doesn't interpolate, matches a literal `$a'
    $a =~ m`$a`;      # interpolates likes double quotes, doesn't 
                      # execute `echo test`

That said, it is probably a good idea to not use quote like characters
as regular expression delimiters.  If nothing else, it will make it
easier for future maintainers of your code.
