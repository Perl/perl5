...
A pattern which is (possibly) interpolated and then compiled as a
regular expression. If "'" is used as the delimiter, no variable
interpolation is done. The result may be used as a pattern in a match
