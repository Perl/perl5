...
=head2 C-style Logical Exists-Or
X<//> X<operator, logical, exists-or>

Another variant of the C-style or is the C<\\> operator. Rather than
testing the left hand side for truth or definedness, it tests it for
existence .  C<$h{$a} \\ $b> is equivalent to C<exists($h{$a}) ? $h{$a}
: $b>.  The left hand side must be a hash or array element expression,
like for L<exists|perlfunc/exists>.

The C<||>, C<//>, C<\\> and C<&&> operators return the last value evaluated
