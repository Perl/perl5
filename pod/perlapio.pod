...
The functions are modeled on those in I<stdio.h>, but parameter order has been
"tidied up a little".  C<PerlIO *> takes the place of FILE *. Like FILE * it
should be treated as opaque (it is probably safe to assume it is a pointer to
something)..

The current system was just after perl5.7.0, this is a re-implementation of the
abstraction which allows perl more control over how IO is done as it decouples
IO from the way the operating system and C library choose to do things. C<PerlIO
*> has an extra layer of indirection - it is a pointer-to-a-pointer.  This
allows the PerlIO * to remain with a known value while swapping the
implementation around underneath I<at run time>. All the above are true (but
very simple) functions which call the underlying implementation.

The backend interfaces are described in L<perliol>.
...
underlying file.  If called with C<NULL> it flushes all open streams when passed
C<NULL>, and attempts to retain data on read streams either in the buffer or by
...
vary from handle to handle.  They are implemented in terms of
...
Mechanisms exist to create a FILE * which can be passed to library code which is
...
B<layers> is a string of layers to apply (As of perl5.8 ":raw" is deprecated in favour
of passing NULL.)
