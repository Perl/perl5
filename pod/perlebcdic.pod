...
characters.  For example, characters such as 'Ù' C<o WITH CIRCUMFLEX> 
...
An example would be that 'À' (or E<Euml>) C<E WITH DIAERESIS> (203) comes 
before 'Î' (or E<euml>) C<e WITH DIAERESIS> (235) on and ASCII machine, but 
the latter (83) comes before the former (115) on an EBCDIC machine.  
(Astute readers will note that the upper case version of 'ﬂ' (or E<szlig>) 
