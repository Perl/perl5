...
freed memory, dereferencing invalid pointers, irregardless of the values
written or read.
...
Exposes data that shouldn't be exposed, e.g. data beyond the end of an
...
The C<.> in C<@INC> issue (CVE-2016-1238) could allow an attacker to
take control if a perl program was run from a directory an attacker
could write to (such as /tmp), or if the program switched to that
directory before calling C<require>.

=item *

Allows an attacker to create, modify, delete or read resources such as
files they otherwise don't have access to.

=item *

Privilege escalation bugs.

e.g. a bug in C<POSIX::setreuid> that failed to properly set the
effective user id.

=back

A security issue in code written in perl that isn't part of the perl
core is a security issue, but probably isn't a security issue in the
perl core, and ought to be reported to the maintainer of that code, not
to us.  The Perl ecosystem is not maintained by any single group, and
this is the policy and contact information for just the small group
maintaining the perl core. We're not the security "team" for the entire
ecosystem, and we don't have the resources to act as a forwarding
service.

A security issue in perl related websites (including C<perl.org>) is a
security issue, but not in the perl core, and again should be reported
directly to the contact address published on the relevant site.
...
already feed C< system "rm -rf /" > to it.
...
However if perl mishandles a large value causing a buffer overflow
...
Rationale: Safe isn't supported as a security mechanism.

=item *

Use of the C<p> and C<P> pack templates.

Rationale: These templates are unsafe by design.
...
Rationale: Stack not reference counted crashes usually occur because
your code is both modifying a reference or glob and using the values
referenced by that glob or reference, so it depends on your code, or
how your code reacts to attacker supplied data.

Note that stack not reference counted bug is still a (difficult to
fix) bug in perl, it just isn't a security issue.
...
bytes, or continuation bytes without a start byte, or over-longs are
considered invalid.
...
Other ways to receive badly encoded SVs usually involve bugs in XS
modules, functions like Encode::_utf8_on() that document loudly
its preconditions, and some C<pack> formats pre-5.32.
...
to be fixed, but won't result in a CVE, they just need to be fixed
...
Rationale: They are completely irrelevant to core perl in terms of
security.
...
i.e. modules under F<cpan/> in the source tree.
...
FIXME: For Win32, do list form of pipe open and list system belong
here?
...
to excessive recursion pre-5.30, and matching may allocate large
amounts of memory, neither of these is a security issue.
...
Reporters are CC-ed during the discussion of their issue.
...
Notify down stream distributors.
...
If we decide it isn't a security issue, we notify the requester, who
has one week to argue that it is. FIXME: vagueness, wording.

After a week, if the requester hasn't convinced us that the issue is a
...
down stream distributors shipping older perls will want to know.

If the issue is public, all fields should be filled in fully.

Requires: Triage.

=head2 Work up fixes

We work up fixes for blead and the two most recent C<maint> branches.
Patches that apply to each of the branches should be included in the
ticket.

The C<maint> fixes should be against the C<HEAD> of the C<maint>
branch.

Note: These should be patches produced by C<git format-patch>.

It may take some time to work up fixes.
...
=head2 Notify down stream distributors

Currently the administrative parts of this are handled by Sawyer.

A publication/release date should be negotiated with down stream
distributors.

There may be some back and forth here, and some iteration on the
fixes, e.g. if they find a bug with the fixes.

A distributor may also ask for support with applying the patches to
older releases, which we may or may not provide, as perl may have
changed too much since an older release for a patch to be backported.
...
Requires: Allocate a CVE ID, Notify down stream distributors, Work up
fixes.

=head2 Publication

Any tickets related to the issue are made public.

The description of the CVE is updated with Mitre.
