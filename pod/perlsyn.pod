...
The C<when> modifier, if followed by a block instead of an expression,
evaluates the content of the block as the condition, and does not
perform a smart match.
...
If you don't watch your expression to be smartmatched, from perl 5.24
you can provide a block to C<when> instead:
