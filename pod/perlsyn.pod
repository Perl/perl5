...
throws an exception with the text C<Well Volunteered>:

    use v5.12;
    sub unimplemented { ... }
    eval { unimplemented() };
    if ($@ =~ /^Well Volunteered at /) {
...
is actually an ellipsis. It has now been renamed the MST operator, in
honor of Matt S. Trout.
