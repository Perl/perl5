...
If C<${^RNG}> is defined, C<rand> obtains eight random bytes from its
provider and scales them in the same way as the default generator.  If the
provider is an object, C<rand> calls its C<rand_bytes> method with C<8> as
its argument.  If the provider is an unblessed code reference, C<rand> calls
it with C<8>.  The provider must return a binary string containing exactly
eight bytes.  The provider is not implicitly seeded by C<rand>.

The provider's state is separate from the state of Perl's default generator.
Localizing C<${^RNG}> changes which provider is used, but does not copy or
restore the state maintained by that provider.
...
If C<${^RNG}> is defined, C<srand> delegates to that provider instead of
applying the normal numeric seed validation.  If the provider is an object,
C<srand> calls its C<srand> method, passing the supplied seed when one was
given.  If the provider is an unblessed code reference, C<srand> calls it
with the supplied seed, or with C<undef> when no seed was supplied.
Therefore C<srand()> and C<srand(undef)> are equivalent for a custom provider,
and non-numeric seed values may be accepted by the provider.  The provider's
return value is returned by C<srand>.

The provider is responsible for maintaining its own state.  In particular,
localizing C<${^RNG}> does not save and restore the provider's state; use the
provider's own seeding or copying facilities when an independent or
repeatable sequence is needed.
