...
file as a Perl script. That is, these statements achieve the same overall
purpose:

    do 'stat.pl';
    eval `cat stat.pl`;

The difference is that C<do> searches C<@INC>, updates C<%INC> if the file
is found, and keeps track of the current filename for error messages. It
also hides the lexicals from its enclosing scope from the evaluated code
(whereas C<eval> exposes them; either behaviour could be what you want).

There is I<no> difference in that both constructs will read, parse and run
the code in the file every time they are encountered, so you probably don't
want to use either of them inside a busy loop.

Unfortunately, C<do> does not provide reliable error detection. Upon error
it returns undef, setting either C<$!> (if it failed to find or read the file)
or C<$@> (if it failed to compile the file).
Note that sometimes a compilation failure causes C<$!> to be set as well.
So you must create a clean slate beforehand and then check C<$@> first:

   {
      local ($!, $@);
      my $result = do 'stat.pl';
      if ($@)
...
However, this is still unreliable: the code in the file may be read and run
successfully, returning undef but leaving C<$!> set to an inconsequential
error. This is I<indistinguishable> from a file read failure to the caller.
The same applies to C<$@> for perls prior to 5.14.

Inclusion of library modules is better done with the
C<use> and C<require> operators, which also do automatic error checking
and raise an exception if there's a problem.
If you need to run code from external files at runtime but require proper
error detection, you will need to write your own solution around C<eval>.
