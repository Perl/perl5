...
The C<goto-EXPR> form expects to evaluate C<EXPR> to a code reference or a
label name.  If it resolves to a code reference, it will be handled
like C<goto-&NAME>, below.  This is especially useful to implement
tail recursion via C<goto __SUB__>.

If the expression resolves to a label name, its scope will be
resolved dynamically.  This allows for computed C<goto>s per
