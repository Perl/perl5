...
L<C<trim>|/trim EXPR>, L<C<uc>|/uc EXPR>,
...
=item trim EXPR
X<trim> X<remove_whitespace> (Perl 7+)

=item trim

=for Pod::Functions strip whitespace from the beginning and end of a string.

This function returns a string with whitespace stripped from the beginning
and end of string. trim() will remove these characters:
...
and other Unicode characters that are flagged as whitespace.
...
L<C<trim>|/trim EXPR> is available only if the trim feature> is enabled
or if it is prefixed with C<CORE::>.  The trim function is enabled
automatically with a C<use v7> (or higher) declaration in the current
scope.

Trim is only available on Perl v7.0 and newer. For older Perl versions
look at the String::Util module for a comprable implementation.
