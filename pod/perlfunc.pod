...
Changes the underlying L<PerlIO> layer of the filehandle.
Returns true on success, otherwise it returns C<undef> and sets C<$!>
(errno).

For example, to use the C<:raw> layer for binary data:

    binmode($fh) or die "Failed to binmode: $!";

    # Or this...
    binmode($fh, ':raw') or die "Failed to binmode: $!";

You may use any layers that L<PerlIO> understands:

    binmode($fh, ':utf8') or die ...
    binmode($fh, ':encoding(UTF-8)') or die ...

On some systems (in general, DOS- and Windows-based systems) C<:raw>
...
In other words: regardless of platform, use C<:raw> binmode() on
binary data, like images, for example.
...
form C<:...>, are called I/O I<layers>.  See L<PerlIO> for a
description of each LAYER type.  The C<open> pragma can be used to
