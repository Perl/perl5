...
There are two types of hooks INC filters, and INCDIR hooks, and there are
three forms of representing a hook: subroutine references, array references,
and blessed objects.
...
If the hook is an object, it should provide an C<INC> method that will
be called as above, the first parameter being the object itself. If it
does not provide an C<INC> method it will be assumed to be a blessed
CODE ref which will simply be executed like an unblessed CODE ref would.
Note that you must fully qualify the method name when you declare an
C<INC> sub, as the unqualified symbol C<INC> is always forced into package
...
reference or an object as described above.  This subroutine or object is
called, again as described above, but the first parameter will be the array
reference. This lets you indirectly pass arguments to the subroutine.
...
A hook may also be used to rewrite the C<@INC> array itself. While this
might sound strange, there are situations where it can be very useful to
do this. Such hooks usually just return undef and do not mix filtering
and C<@INC> modifications. While in older versions of perl having a hook
modify C<@INC> was fraught with issues and could even result in segfaults
or assert failures, as of 5.37.7 the logic has been made much more robust
and the hook now has control over the loop iteration if it wishes to do
so.
...
hook returns the next slot in @INC that will be checked will be the
integer successor of value in $INC (or -1 if it is undef). For example
the following code

    push @INC, sub {
        splice @INC, $INC, 1; #remove ourself from @INC
...
would install a sub into @INC that when executed as a hook (by for
...
installed hook imediately, or to inspect any changed items in @INC to
