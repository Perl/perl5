...
locale ability.  Some caveats apply, see L</Multi-threaded> below.

Starting in Perl 5.46, perl may be Configured to emulate thread-safe
locales on systems that lack them natively.  In earlier perls, or
without this being activated, do NOT use this pragma in scripts that
have multiple L<threads|threads> active.  The locale in these cases is
not local to a single thread.  Another thread may change the locale at
any time, which could cause at a minimum that a given thread is
...
operations cause perl itself to change the locale.  You are vulnerable
simply by having done a S<C<"use locale">>.
...
including Darwin, claim to have it, but are buggy as of September 2026, so
...
the value is 2 if thread-safe locales are being emulated.

Thread-safe operations are supported automatically in Windows starting
in Visual Studio 2005, and in MingW compiled to use UCRT (the
Universal C Run Time library), and in systems perl believes are actually
compatible with POSIX 2008.  Note that C<${^SAFE_LOCALES}> will be 0 on
threaded builds on platforms that Perl knows to have buggy
implementations.

Be aware that writing a multi-threaded application will not
automatically be portable to a platform which lacks native thread-safe
locale support and which haven't been Configured to emulate them. 
On systems that perl believes do have it, you automatically get this
behavior for threaded perls, without having to do anything.  If for some
reason, you don't want to use this capability (perhaps the POSIX 2008
support turns out to be buggy on your system (in which case you should
let us know by filing a ticket via XXX), and you don't want to emulate
thread-safe locales, you can manually compile Perl to use
...
in some situations.  If these are buggy, you can instead or additionally
pass the following to F<Configure>:
C<-Accflags='-DNO_POSIX_2008_LOCALE'>.  This will also keep the code
from using thread-safe locales.  C<${^SAFE_LOCALES}> will be 0 on
...
entirely in Perl (minus the caveats given in this section).  Information
for XS module writers is given in L<perlclib/Dealing with locales>.
