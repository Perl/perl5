...
C<CPAN::Version> (improved support for multidot version notation)
...
Digest::file upgraded to version 1.00
...
DynaLoader now implements C<dl_unload_file> on Win32
...
FindBin upgraded to version 1.47. C<FindBin> now works better with
...
Pod::Html upgraded to version 1.0504. HTML output will now correctly link
...
Storable upgraded to version 2.15. This includes C<STORABLE_attach> hook
functionality added by Adam Kennedy, and more memory frugal storing under
ithreads, by utilising the ithreads cloning tracking code.
...
threads::shared upgraded to version 0.94, which is solely documentation
changes. An improved implementation of C<threads::shared> is available on
...
most are now unused. Perl can now be compiled with C<-DPERL_DONT_CREATE_GVSV>
to avoid creating these empty scalars. This will significantly decrease the
number of scalars allocated for all configurations, and the number of scalars
that need to be copied for ithread creation. Whilst this option is not
binary incompatible with existing perl installations, it does change a
long-standing previous assumption about the internals, hence it is not enabled
by default, as some third party code may rely on the old behaviour.

We would recommend testing with this configuration on new deployments of
perl, particularly for multithreaded servers, to see whether all third party
...
Weak reference creation is now I<O(1)> rather than I<O(n)>. Weak reference
deletion remains I<O(n)>, but if deletion only happens at program exit, it
...
real memory usage on mutli-user systems.
...
C<Configure> will now probe for the presence of C<futimes> and whether
C<sprintf> correctly returns the length of the formatted string, which will
both be used in perl 5.8.9.
...
of this has been identified and fixed.
...
C<//g> matches used to loop infinitely on tainted data. Dave Mitchell
resolved this.
...
String buffer allocation is now rounded up to the next multiple of 4 (or
8 on platforms with 64 bit pointers). This should reduce the number of calls
to C<realloc> without actually using any extra memory.

The C<HV>'s array of C<HE*>s is now allocated at the correct (minimal) size.
Compile with C<-DPERL_USE_LARGE_HV_ALLOC> to use the old, sloppier, default.
...
it is impossible to produce any meaningful dump of their contents.
