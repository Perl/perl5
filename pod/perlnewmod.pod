...
=item Start with F<perlnewdist>

The F<perlnewdist> utility is the recommended way to start a new Perl
distribution. Give it the name of the primary module:

    perlnewdist Foo::Bar

It creates the standard directory layout, a starter module, a test file,
Makefile.PL, README, and Changes. The generated test should pass immediately.
Use C<--with ::Name> to create additional modules below the primary module,
and C<--xs> when the distribution needs native C code.

On the first run, F<perlnewdist> asks for the author name and email address.
It uses Git configuration to prefill those questions when possible and can
save the answers in F<~/.perlnewdist>.

=item Advanced generation with F<h2xs>
...
F<h2xs> remains available for advanced work, particularly when generating
XS from C header files or when exact control over the generated extension is
needed. Its interface contains many historical options and is not normally
the best starting point for a new pure-Perl distribution.
...
F<perlnewdist> will provide a stub for you to fill in; if you're not sure
about the format, look at L<perlpod> for an
...
you the results of the tests. Again, F<perlnewdist> provides a test
...
Once again, F<perlnewdist> has created the standard distribution files for
you. It produces the standard C<Makefile.PL> you see when you download and
...
L<perlmod>, L<perlmodlib>, L<perlmodinstall>, L<perlnewdist>, L<h2xs>, L<strict>,
