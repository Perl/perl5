...
=item Enhanced C</xx> modifier in regular expression pattern matching

Introduced in Perl 5.43.8.

Using this feature triggers warnings in the category
C<experimental::enhanced_xx>.

This feature enables bracketed character classes to be split across
multiple lines, and to contain comments, and adds warnings if it appears
a C<#> is interpreted as beginning a comment when it was meant to be
taken as a literal character.
