...
perldelta - what is new for perl v5.45.1

=head1 DESCRIPTION

This document describes differences between the 5.45.0 release and the 5.45.1
release.

If you are upgrading from an earlier release such as 5.44.0, first read
L<perl5450delta>, which describes differences between 5.44.0 and 5.45.0.
...
The new F<perlnewdist> utility provides a beginner-friendly way to create the
initial files for a Perl distribution. It is a thin
wrapper around L<h2xs>: it selects modern defaults, gathers author information,
and can create additional pure-Perl modules or initialize a Git repository,
while delegating the distribution scaffolding to F<h2xs>.
...
F<h2xs> now warns when run without C<-A> or C<--omit-autoload>. Autoloading is
retained for backwards compatibility, but is discouraged for new modern Perl
projects. New users should generally start with F<perlnewdist>; F<h2xs>
remains available for advanced XS, C-header, and compatibility workflows.
...
XXX Generate this with:
  ...
  perl Porting/acknowledgements.pl v5.45.1..HEAD
