...
=head2 C<.bs> files are no longer loaded

L<XSLoader> and L<DynaLoader> no longer load C<.bs> files.  This feature was
only used for L<DB_File> on NeXT, support for which was dropped in 5.22.0.  On
CPAN, the only distribution with a C<_BS> file is C<HTML::Embperl>, which is
long abandoned.
