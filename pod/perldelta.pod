...
may be found in the C<Changes> files that accompany the Perl source
...
map() could get pathalogically slow when the result list it generates
...
with C<Configure -Uusemymalloc ...> as discussed in the C<INSTALL> file.
...
the various dbm libraries.  SunOS hints preserve behavior on that
platform.
...
The C<INSTALL> file has been expanded to cover various issues, such as
64-bit support.
