...
Various bugs in C<POSIX::mbtowc> were fixed.   It could read past the
end of the string if the second parameter was wrong, and the returned
wide character likely would be garbage.

Various bugs in C<POSIX::wctomb> were fixed.   It would segfault if the
string parameter was shared or hadn't been pre-allocated with a string
of sufficient length to hold the result.
