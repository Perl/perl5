...
=head2 strict only allows refs, subs, vars

Earlier versions of Perl did not check the parameters implicitely passed
to its "import" and "unimport" routine.  This caused idiom such as:

 use strict qw(@ISA);
 @ISA = qw(Foo);

to B<not> cause any error condition to be raised.  This idiom however raised
the false expectation that strict refs, vars and subs were being enforced.
They are B<not> when using this idiom.

Starting from 5.8.1, the above idiom B<will> cause an error to be raised.
...
B::Bytecode - in much better shape than it used to be.  Still far
...
available.  This is intentional.  They never should have been
...
=head2 Cygwin 1.5 sysio.t

The one test

  op/sysio.t                                39    1   2.56%  39

is failing in 1.5.3s for some yet unknown reason.
...
lib/ExtUtils tests that invoke the compiler.  The warning, however, is
