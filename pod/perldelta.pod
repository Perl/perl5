...
perldelta - what is new for perl v5.39.11

=head1 DESCRIPTION

This document describes differences between the 5.39.10 release and the 5.39.11
release.

If you are upgrading from an earlier release such as 5.39.9, first read
L<perl53910delta>, which describes differences between 5.39.9 and 5.39.10.
...
The hints file has been updated to handle the oneAPI DPC++/C++ compiler from Intel.
...
Tne delimiter C<SYRIAC COLON SKEWED LEFT/RIGHT> pair has been removed
...
visually confusing [GH #22228].  The change was actually to forbid any
right-to-left delimiters, but this pair is the only current instance
that meets this criterion.  By policy, this change means that the
C<extra_paired_delimiters> feature cannot be considered to have been
stable long enough for its experimental status to be removed.  This in
turn means that it is no longer automatically enabled by the 5.40
feature bundle.  The bottom line is that the 5.40 cumulative perldelta
should not reference this feature except for the removal of this pair.
...
XXX Generate this with:
  ...
  perl Porting/acknowledgements.pl v5.39.10..HEAD
