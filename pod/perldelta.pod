...
Regular expression debugging information available through the C<re>
module now works when compiling Unicode property wildcard subpatterns.
See L<perlunicode/Wildcards in Property Values>.  Some potentially
voluminous output requires a new option, WILDCARD, to be specified to
enable.  See L<re/WILDCARD>.
