...
memory needed to regenerate Unicode data.  A full Unicode C<mktables> run is
approximately 23% faster and uses 17% less peak memory.  The MPH squeeze path
is also substantially faster, at the cost of a small increase in the
generated data size.  Parallel builds now start C<mktables> as soon as
miniperl and F<Config.pm> are available, allowing Unicode table generation to
overlap extension builds.  Previously, it waited for every non-XS extension
to finish.
