...
Complete rewrite.  As a side-effect, no more refuses to startup when
run by root.
...
=head2 Cygwin 1.5 sysio

The F<op/sysio> test has failures failing in 1.5 for some yet unknown reason.
