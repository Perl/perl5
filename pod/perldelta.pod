...
This document describes differences between the 5.005 release and the 5.6.2
release.

=head1 Summary of changes between 5.6.1 and 5.6.2

The 5.6.2 release is aimed at providing only a minimal update to perl 5.6.1,
which wasn't being compatible with some of the newest compilers (most
notably gcc 3.x), libraries and operating systems that have appeared since
5.6.1 was released.

Note also that perl no longer installs itself as F</usr/bin/perl> by
default if it exists. (see the F<INSTALL> file.)

The following modules have been upgraded :

=over

=item Data::Dumper 2.121

=item DB_File 1.806

=item ExtUtils::MakeMaker 6.17
...
=item Test::Builder 0.17

=item Test::More 0.47

=item Test::Simple 0.47
...
B<Note>: If you want robust Unicode support in Perl, you should use the
5.8.x track of Perl.
...
B<NOTE>: This feature isn't stable in the 5.6.x series. If you want to use
multiple interpreters and/or threads, you should use the 5.8.x track of
perl.
...
There may also be information at L<http://www.perl.org/>.
