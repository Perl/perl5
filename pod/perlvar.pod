...
If its value is an object, C<rand> calls its C<rand_bytes> method with the
number of bytes it needs, and C<srand> calls its C<srand> method.  If its
value is an unblessed code reference, C<rand> calls it with the requested byte
count and C<srand> calls it with either the supplied seed or C<undef> when no
seed was supplied.  Thus C<srand()> and C<srand(undef)> are equivalent for a
custom generator.  The random-value callback or method must return a binary
string containing exactly the requested number of bytes.  The provider is
responsible for maintaining its own state; localizing this variable does not
copy or restore that state.
