...
In Perl 5.44.0 the C</p> match operator flag does nothing, and the
C<${^PREMATCH}>, C<${^MATCH}>, and C<${^POSTMATCH}> are always populated,
behaving exactly like C<$`>, C<$&>, and C<$'>.
...
In Perl v5.42 and earlier, it is only guaranteed to return a defined
value when the pattern was compiled or executed with the C</p> modifier.
In Perl v5.44, the C</p> modifier does nothing, so C<${^MATCH}> is always
available, and does the same thing as C<$MATCH>.

This variable was added in Perl v5.10.0.

See L</Performance issues> above for historical context.
...
In Perl v5.42 and earlier, it is only guaranteed to return a defined
value when the pattern was compiled or executed with the C</p> modifier.
In Perl v5.44, the C</p> modifier does nothing, so C<${^PREMATCH}> is always
available, and does the same thing as C<$PREMATCH>.
...
In Perl v5.42 and earlier, it is only guaranteed to return a defined
value when the pattern was compiled or executed with the C</p> modifier.
In Perl v5.44, the C</p> modifier does nothing, so C<${^POSTMATCH}> is always
available, and does the same thing as C<$POSTMATCH>.
