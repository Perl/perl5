...
If you want to print perl scalar which you get as C<SV*>, you can use
C<SVf> format and pass scalar via C<SVfARG()> macro.  See example:

 SV *sv = ERRSV;
 croak("This croaked because: %" SVf "\n", SVfARG(sv));
...
Note that C<SVf> format correctly handles C<SVfUTF8> flag and therefore
it is safe to use it for Unicode strings.
