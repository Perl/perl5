...
convenience, and the C way for precision.  The shell way has 2- and
3-argument forms, which have different semantics for handling the open mode
and filename.  The 3-argument form should always be used because the
legacy form can lead to insecure code if the filename contains one of the
special characters that control the mode.  Therefore the tutorial won't
show examples of the 2-argument form which should be avoided.
...
the second defines the mode and the third what to open.  C<open> returns true
when it works or returns a false value when it fails and sets the special
variable C<$!> to reflect the system error.  If the filehandle was previously
opened, it will be implicitly closed first.
