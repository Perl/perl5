...
binary is now C<sperl7.8.>I<n> (C<sperl7.8.4> for this release). C<suidperl>
is installed as a hard link to C<perl>; both C<suidperl> and C<perl> will
invoke C<sperl7.8.4> automatically the set uid binary, so this change should
...
The Perl debugger (F<lib/perl7db.pl>) can now save all debugger commands for
