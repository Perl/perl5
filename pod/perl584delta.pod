...
the set uid binary is now called C<setuidperl>, with C<suidperl> installed
as a hard link to C<perl>. Both C<suidperl> and C<perl> will invoke
C<setuidperl> automatically on a set uid script, so this change should be
...
made. Perl should now build on Windows 95. On Windows Borland's C compiler can
