...
F<perlnewdist> creates the initial files for a new Perl distribution. It is
the recommended starting point for a new pure-Perl module or application.
It creates the standard module, test, documentation, and build files, and
can initialize a Git repository.

=item L<h2xs>

F<h2xs> converts C header files into XS modules, and will try and write
as much glue between C libraries and Perl modules as it can. It is the
lower-level tool to use when working directly with XS, C headers, or the
advanced extension-generation options; new pure-Perl distributions should
normally start with F<perlnewdist>.
...
L<perlbug>, L<h2ph>, L<perlnewdist>, L<h2xs>, L<enc2xs>,
