...
X<subroutine, declaration> X<sub>
    ...
    sub NAME BODY 		  # A declaration and a definition.
    sub NAME(PROTO) BODY 	  #  ditto, but with prototypes
    sub NAME : ATTRS BODY 	  #  with attributes
    sub NAME(PROTO) : ATTRS BODY  #  with prototypes and attributes

To define an anonymous subroutine at runtime:
X<subroutine, anonymous>

    $subref = sub BODY;                  # no proto
    $subref = sub (PROTO) BODY;          # with proto
    $subref = sub : ATTRS BODY;          # with attributes
    $subref = sub (PROTO) : ATTRS BODY;  # with proto and attributes
...
modified or removed in future versions of Perl.  It is almost certain
that future versions of Perl will have more complex signature facilities.
...
pragmatic declaration, C<use feature 'simple_signatures'>, and it will
produce a warning unless the "experimental::simple_signatures" warnings
category is disabled.

The signature is part of a subroutine's body.  Normally the body of a
subroutine is simply a braced block of code.  When using a signature, the
signature is a parenthesised list of variable names that goes immediately
before the braced block.  The variables named by the signature are
declared as lexical variables (as if by C<my>).  When the subroutine is
called, the argument list is assigned to the list of lexical variables,
so that the variables get copies of the argument values.  For example,
...
The equivalence is quite precise.  The same kinds of things can appear
in the signature both ways, and the assignment of the argument list
has the same semantics.  An argument can be ignored by putting C<undef>
in the signature in place of a parameter name:

    sub foo ($first, undef, $third) {
	return "first=$first, third=$third";
    }

The signature does not perform any checking of the number of arguments
supplied to the subroutine.  If fewer arguments were supplied than the
number of formal parameters, the unsatisfied parameters will be undefined
if scalar, or empty if array or hash.  If more arguments were supplied
than can fit into the formal parameters, the excess arguments are ignored.

An array or hash parameter slurps up all the remaining arguments.
Any parameters listed later in the signature won't have any arguments
available to them, and so will end up undefined or empty.

The arguments are still available in the special array variable C<@_>, in
addition to the lexical variables of the signature.  There is a difference
between the two ways of accessing the arguments: C<@_> I<aliases> the
arguments, but the signature variables get I<copies> of the arguments.
So writing to a signature variable only changes that variable, and has
no effect on the caller's variables, but writing to an element of C<@_>
modifies whatever the caller used to supply that argument.

There is a syntactic ambiguity between signatures and prototypes (see
L</Prototypes>), because both start with an opening parenthesis and
...
When signatures are enabled, the prototype syntax becomes much more
restricted: only a parenthesised group of ASCII punctuation characters
will be treated as a prototype.  In particular, any alphabetic or
space character will prevent the group being treated as a prototype.
It will then be parsed as a signature instead.  Beware that a completely
empty group, C<()>, is a prototype, even though it would also be valid
signature syntax.  To write an empty signature, put a space between the
parentheses.  This is a bit pointless, because the empty signature has
no effect.  It is also possible to construct a non-empty signature such
as C<($__)> that looks like it could be a prototype and will be treated
as one; again, adding a space will force interpretation as a signature.
...
The prototype must come before the signature.  Attributes, if present,
must also come before the signature (and after the prototype).
