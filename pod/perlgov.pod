...
B<NOTE>: Remember that every new stable release triggers an election, so the
document you are looking at is B<almost certainly out of date> unless you are
looking at L<the most up to date copy in the C<blead> branch of perl's git
repository|https://github.com/Perl/perl5/blob/blead/pod/perlgov.pod>.
You can also see membership changes over time using the commit history there.
...
=head2 Core Team Members

=head3 Active Members

These are the active members of the Perl Core Team as of the last update
on April 11, 2026:
...
=head3 Inactive Members

These are the inactive members of the Perl Core Team as of the last update
on July 1, 2023:
