...
=item * -leak-check=files

=item * -leak-check=ipc

Check for leaks of SysV IPC objects and temporary files.
See L<Leak checks>.
...
F<harness> accepts C<-leak-check=files> to check for file leaks,
C<-leak-check=ipc> to check for leaks of SysV IPC objects, or
C<-leak-check> to check for both types of leaks.

The file check complains if extra files are found in the F<t>
directory, or in a new directory which is assigned to C<TMPDIR> so
that L<File::Temp> will create temporary filenames there.

The SysV IPC check snapshots the results of C<ipcs>, compares the
results as it runs each test, and complains if new objects are
created but not released by a test.

Either of these tests can be broken by other system activity - if you
have another process creating and destroying IPC objects then the IPC
checks will report errors when there are none.  If you're editing
files in F<t/> editor temp files might also cause problems.
