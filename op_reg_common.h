 ...
 *    2019, 2020, 2021, 2022 by Larry Wall and others
 *
...
/* This tells where the first of these bits is.  Setting it
 * to 0 saved cycles and memory.  I (khw) think the code
 * will work if changed back, but haven't tested it */
 ...
 * to ensure that the options are mutually exclusive */
/* Make sure to update ext/re/re.pm and regcomp.sym (as
 * these are used as offsets for various node types, like
 * POSIXD vs POSIXL, etc) when changing this! */
typedef enum {
 ...
 * would need to be for proto.h to understand it */
...
set_regex_charset(U32 * const flags, const regex_charset cs)
{
    /* Sets the character set portion of 'flags' to 'cs',
     * which is a member of the above enum */
...
#define PMf_SPLIT               (1U<<11)
    ...
    PMf_FOLD != RXf_PMf_FOLD ||                     \
    PMf_EXTENDED != RXf_PMf_EXTENDED ||             \
    PMf_EXTENDED_MORE != RXf_PMf_EXTENDED_MORE ||   \
    PMf_KEEPCOPY != RXf_PMf_KEEPCOPY ||             \
    PMf_SPLIT != RXf_PMf_SPLIT ||                   \
    PMf_CHARSET != RXf_PMf_CHARSET ||               \
    PMf_NOCAPTURE != RXf_PMf_NOCAPTURE ||           \
    PMf_STRICT != RXf_PMf_STRICT
#   error RXf_PMf defines are wrong
#endif

/*  Error check that haven't left something out of this.  This isn't
 *  done directly in the #define because doing so confuses
 *  regcomp.pl.  (2**n - 1) is n 1 bits, so the below gets the
 *  contiguous bits between the beginning and ending shifts */
