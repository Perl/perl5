    ...
    case CXt_LOOP_LAZYSV: /* string increment */
    {
        ...
        if (DO_UTF8(end) && IN_UNI_8_BIT)
            ...
            goto retno;
                ...
                /* We are looping n-at-a-time and the range isn't a multiple of
                   n, so we fill the rest of the lexicals with undef.
                   This only happens on the last iteration of the loop, and we
                   will have already set up the "terminate next time" condition
                   earlier in this for loop for this call of the ITER op when we
                   set up the lexical corresponding to the last value in the
                   range. Hence we don't goto retno (yet), and just below we
                   don't repeat the setup for "terminate next time". */
                pad_it = TRUE;
            }
    ...
    case CXt_LOOP_LAZYIV: /* integer increment */
    {
        ...
        else
        {
            ...
            *itersvp = newSViv(cur);
            ...
            if (UNLIKELY(pad_it)) {
                /* We're good (see "We are looping n-at-a-time" comment above).
                 */
            }
